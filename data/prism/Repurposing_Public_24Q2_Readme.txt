...
The screen is executed using 384-well assay-ready compound plates where each plate has 32 negative (DMSO) and positive (20uM Bortezomib) control wells beside ~300 test compounds. Each compound plate is screened in three biological replicates with a randomized plate map for each replicate. The output of the Luminex detection step at the assay endpoint (5 days) is the fluorescence intensity levels for each bead-replicate pair. The code and more detailed description of the data processing and QC steps are available at https://github.com/broadinstitute/repurposing and https://github.com/broadinstitute/prism_data_processing. Below, we give a brief description of those steps by pointing out which files in the release package have the output of each step.


    1. The median fluorescence intensity (MFI) of each bead-replicate pair is calculated and log2-transformed. (Repurposing_Public_24Q2_LMFI_matrix.csv)
    
    2. LMFI values are normalized to inert controls in the same condition on the same plate. (LMFI.normalized column in the Repurposing_Public_24Q2_LMFI_NORMALIZED.csv)
    
    3. For each detection plate x cell line pair, a control separation filter is applied for quality control purposes. In particular, for each pair, the Bayes error of a classifier to separate negative and positive control wells is computed along with the dynamic range of the signal (defined as the difference between median LMFI.normalized of DMSO and  Bortezomib wells). Any pair with an error rate more than 5% or dynamic range less than 2 are flagged as QC failures. The flagged cell line x detection plate pairs are filtered out as well as any cell lines x compound plate with fewer than two passing replicates from the downstream analysis (see Repurposing_Public_24Q2_QC_table.csv).
    
    4. Log2-fold-change is calculated for each condition relative to negative-control wells from the same cell line on the same detection plate, positive-control and negative-control treatments are removed. (LFC column of Repurposing_Public_24Q2_LFC.csv)
    
    5. ComBat batch correction method (DOI: 10.1093/biostatistics/kxj037) is applied for each treatment condition (compound plate-well combination), considering log2-viability values as probes and cell line pool-replicate combinations as batches. (LFC_cb column of Repurposing_Public_24Q2_LFC.csv)
    
    6. Log2-fold changes are median-collapsed across plates for each cell line perturbation combination (Repurposing_Public_24Q2_LFC_COLLAPSED.csv). 

    7. Finally collapsed log2-fold changes are casted into matrix form and concatenated with the previously published PRISM Repurposing Primary dataset for convenience (Repurposing_Public_24Q2_Extended_Primary_Data_Matrix.csv). For the meta-data of this matrix, please see Repurposing_Public_24Q2_Extended_Primary_Compound_List.csv.
...
# QC TABLE AND LOWER LEVEL DATA

## Repurposing_Public_24Q2_LMFI_matrix.csv : Contains the log2 median fluorescence intensity values for each cell line (or control barcode) and perturbation well. This is the raw data coming from the luminex detection.
