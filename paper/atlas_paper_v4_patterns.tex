% DI Atlas paper — v4, formatted for Patterns (Cell Press)
...
%% ====================================================================
%% Graphical abstract (Patterns requires this)
%% ====================================================================
\begin{figure}[h]
\centering
...
%% ====================================================================
%% Highlights (Patterns requires up to 4, each <= 85 characters)
%% ====================================================================
\noindent\textbf{Highlights}
\begin{itemize}
\setlength{\itemsep}{0pt}
\item Drug mechanism class structures cross-context consistency ($\eta^2 = 0.20$)
\item Atlas of ${\sim}$55{,}000 perturbations spanning three measurement platforms
\item Metric separates inconsistent effects from merely weak ones
\item Eleven pre-registered tests; failures constrain what the metric captures
\end{itemize}
...
%% Keywords
%% ====================================================================
\noindent\textbf{Keywords:} direction instability, perturbation biology,
context-dependence, pre-registration, Cell Painting, L1000, CRISPR
screening, drug mechanism of action, essentiality, geometric metric

\vspace{2em}

%% ====================================================================
%% Summary (Patterns: single paragraph, <= 150 words)
%% ====================================================================
\noindent\textbf{Summary}

\begin{quote}
Whether a perturbation produces consistent effects across cell
types determines if laboratory findings transfer to patients---yet no
existing metric separates inconsistent effects from merely weak ones.
Selectivity metrics count how many targets a drug hits; direction
instability (DI) measures whether downstream effects depend on cell
type.  Across ${\sim}$55{,}000 drugs and genetic perturbations, DI
reveals that mechanism of action structures transferability.  Drugs
targeting core cellular machinery---mTOR inhibitors, HDAC
inhibitors---act the same way regardless of cell type (low~DI), while
receptor-signaling drugs---serotonin agonists, histamine
agonists---produce cell-type-dependent effects (high~DI).  A highly
selective drug can still have high DI if its target pathway is wired
differently across cell types---selectivity alone does not predict
transferability.  Essential genes produce uniform disruption across cell types,
extending the pattern from drugs to genetic perturbations.  The atlas provides
a lookup resource for whether a perturbation's effects will transfer to
a new context.
\end{quote}

\section*{The bigger picture}
When developing drugs or studying gene function, a central question is
whether an intervention works the same way across cell types.  A drug
that kills cancer cells in one cell line may fail in patients if its
mechanism depends on the specific cellular context.  Gene knockout
screens in one cell line may not generalize to others.  Despite the
importance of this question, no magnitude-immune metric exists for
measuring cross-context consistency at perturbation-atlas scale
\citep{tower2026confound}.

Direction instability addresses this by comparing the directions of a
perturbation's effects across contexts, ignoring their magnitude.  This
study presents an atlas-scale characterization of DI across
${\sim}$55{,}000 perturbations spanning gene expression and cell
morphology, paired with eleven pre-registered hypothesis tests that
characterize what the metric captures.  The results reveal that DI is
structured by pharmacological mechanism: drugs targeting core cellular
machinery act consistently across cell types, while those acting through
receptor-mediated signaling produce context-dependent effects.  Essential
genes produce stereotyped disruption regardless of cell type, yielding
low DI---the opposite of the pre-registered prediction.  These findings inform drug development by identifying compounds whose
mechanisms transfer across tissues, and genetic screening by revealing
which knockouts are cell-type-specific.
Beyond these biological results, the pre-registered design illustrates
how geometric metrics in computational biology can be rigorously
characterized.
...
\citep{bray2016cell}, and single-cell modalities \citep{tahoe2025}.
...
(Section~\ref{sec:atlas}).  A companion study \citep{tower2026confound}
established this immunity and showed that DI predicts
cross-cell-line mechanism transport (pre-registered,
construction-neutral AUROC $= 0.945$ on 8{,}949 LINCS drugs).  Because DI is defined through cosine
geometry, that study validated it through three independent biological
channels that do not involve cosine similarity.  These include Jaccard
overlap of perturbed gene sets (AUROC $= 0.957$), breadth of target gene
expression across tissues ($\rho = -0.17$, $p < 0.001$), and PRISM
cell-viability correlation ($\rho = 0.261$, $p < 0.001$).  A separate
construct-validity study \citep{tower2026validity} extended these results
to Perturb-seq and JUMP-CP morphology, showing that domain-specific
validity conditions preserve global DI rankings ($\rho > 0.91$ in all
three domains) while upgrading inferential warrant at the item level.
The present study takes DI's validity as established and asks a
different question: what does DI capture, and what does it fail to
capture, across modalities and perturbation types?
...
The results sharply constrain what DI measures.  Cross-modal concordance confirms
...
signatures are anti-correlated across contexts.  Tahoe-100M drug DI
...
and exceeds 1 when signatures are anti-correlated.  By
...
1 & Viability concordance & 75 & $\rho = 0.359$ & 0.0016 & PASS (exploratory) \\
\bottomrule
\end{tabular}
\end{table}
...
measures.  Batch~3 results (Experiments~F, G, H) are reported in
Section~\ref{sec:batch3} and Table~\ref{tab:results}.}
\label{fig:forest}
\end{figure}
...
constraint \citep{hart2015crispr, meyers2017crispr,
funk2022phenotypic, ramezani2025periscope}, and DI indexes the
complement of that constraint.
...
In short: DI variation across genes and drugs is not explained by
existing gene annotations, and drug selectivity remains unresolved at
$n = 75$.  Three reviewer-prompted experiments
(Section~\ref{sec:batch3}) extend this characterization.
...
cell-type-specific biology.  It is orthogonal to essentiality,
...
opposite sign.  The mechanistic explanation is plausible, connects to
established results in functional genomics
\citep{funk2022phenotypic, ramezani2025periscope}, and is independently
corroborated by a separate study that found universal-essential
knockdowns diverge rather than converge across cell types in Perturb-seq
data ($d = 0.60$, $p_{\mathrm{perm}} < 10^{-4}$;
\citealp{tower2026validity}); it was, however, generated after observing
the data.  Prospective testing of a
...
\paragraph{Lead contact.}
Requests for information and resources should be directed to the lead
contact, Elliot Tower (\texttt{elliot@elliottower.ai}).
...
(DOI: 10.5281/zenodo.21223667) under a CC-BY-4.0 license.
...
DI computation, magnitude correction, and bootstrap confidence intervals
are described in Section~\ref{sec:atlas}.  Pre-registration protocol and
deviations are documented in Section~\ref{sec:integrity}.  All analysis
scripts reproduce the results from source data; scripts and frozen
pre-registration documents are included in the Zenodo deposit.

\subsection*{Quantification and statistical analysis}

All hypothesis tests use partial Spearman correlation computed by
Pearson-residualizing both variables against the covariate matrix (with
intercept), then computing Spearman's~$\rho$ on the residuals
(Section~\ref{sec:atlas}).  Confidence intervals use the Fisher
$z$-transform with degrees of freedom adjusted for the number of
covariates.  Bonferroni correction is applied within each batch
($\alpha = 0.0125$ for Batch~1's four primary tests; $\alpha = 0.0167$
for Batch~2A and Batch~3, three tests each).  A uniform CI lower-bound
gate ($\pm 0.05$) prevents trivially small effects from passing at
large~$n$.  Permutation nulls (1{,}000 shuffles) are used for the
knockout vs.\ overexpression test and the MoA stratification test.
Experiment~F uses one-way ANOVA with partial $\eta^2$ on
residualized DI.  All statistical analyses were performed in Python~3.11
using SciPy~1.12, NumPy~1.26, pandas~2.2, and scikit-learn~1.4.
